...
class ExchangeAuthService:
    ...
    def __init__(self):
        ...
        self.server_private_crypto_service = CryptographyService(key=settings.SERVER_FERNET_KEY_SECRET.encode('utf-8'))
    ...
    def validate_token(self, token: str) -> Tuple[bool, Optional[str]]:
        ...
        necessary_fields = ["uuid_id", "email", "access_level", "is_active", "id", 'firebase_id']
        ...
        important_fields = ["uuid_id", "email", "access_level", "is_active", "id", 'firebase_id']
        ...
        if token["access_level"] not in [1, 2]:
    ...
    def encrypt_user_cookie(self, user_cookie: UserCookieModel) -> str:
        ...
        encrypted_cookie = self.server_private_crypto_service.encrypt(user_cookie_json.encode('utf-8'))
